...
Temperature medie attuali nei vari continenti durante l'anno

```{r fig.height=2.5}
continent_temp_2002_2012 <- df %>% filter(Year %in% (2002:2012)) %>% group_by(continent) %>% 
  summarise(AverageTemperature=mean(AverageTemperature, na.rm=TRUE))

ggplot(continent_temp_2002_2012, aes(x = reorder(continent, AverageTemperature), y = AverageTemperature)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label=format(round(AverageTemperature, 1))), hjust = +1.5, size = 5, colour="white") +
  coord_flip() +
  labs(y= "Average year temperature (°C)", x = "Continent") +
  theme_minimal() +
  theme(panel.grid.major=element_blank()) +
  theme(panel.border=element_blank()) +
  theme(axis.ticks=element_blank()) +
...
df_pond <- df %>% group_by(Year) %>% 
  ...
  mutate(AverageTemperature=rollmean(AverageTemperature, k = 10,  fill = NA, align="right"),
         AverageTemperatureUncertainty=rollmean(AverageTemperatureUncertainty, k = 10,  fill = NA, align="right")) %>% 
  filter(Year > 1758) %>%
...
world_anim <- 
  ...
  tm_fill("diff_temp", title="Temp. difference since 1860", style = "fixed",
        breaks = c(0, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8, Inf),
        palette = c("#00ACEC", "#7AC3FF", "#a6e0ff", "yellow", "orange", "red", "#BA0000"),
        colorNA = "gray") +
  tm_borders()
...
Dal grafico si può notare che effettivamente in media la temperatura è costantemente aumentata nel periodo analizzato con una variazione nella media di circa 3°C (+18%) ai giorni nostri rispetto al 1860.
...
L'aumento più vertigionoso si nota successivamente al 1870, periodo di piena rivoluzione industriale e tra il 1970-2010, anni in cui la produzione di energia proveniente da combustibili fossili è aumentata del 140%, le emissioni dei trasporti sono aumentate del 120% e le emissioni delle industrie del 65% ([Fonte](https://www.ipcc.ch/site/assets/uploads/2021/03/ar4-wg3-spm-1.pdf)).
...
```{r}
cor.test(emissions_vs_temp$Emissions, emissions_vs_temp$AverageTemperature)
```
...
cor.test(emissions_vs_temp_country$diff_temp, emissions_vs_temp_country$total_emissions_1970_2012)
```
...
- L'aumento di temperatura rispetto al 1860 è considerevole ed è di circa 3° (+18%)
- L'aumento di temperatura è strettamente correlato all'aumento di produzione di CO2 da parte delle industrie (aumentato di 200 volte negli ultimi 160 anni)
- L'aumento di temperatura non è uniforme tra i paesi e l'aumento di temperatura in un paese non è strettamente correlato alla quantità di CO2 che produce
